...
function finishQuiz() {
    ...
    // 从打卡任务中获取准确的新单词和复习单词数量
    if (clockInSystem.dailyTask && clockInSystem.dailyTask.words) {
        newWords = clockInSystem.dailyTask.words.newWords.length;
        reviewWords = clockInSystem.dailyTask.words.reviewWords.length;
    } else {
        //  fallback: 如果没有打卡任务数据，使用原有逻辑
        newWords = appState.trainingWords.filter(w => w.status === 'new' || w.score === 0).length;
        reviewWords = totalWords - newWords;
    }
    ...
    if (elapsedMinutes < 12) {
        ...
        <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
            ...
            <div>
                ...
                <div style="color: #666; font-size: 14px;">用时 < 10分钟</div>
            </div>
        </div>`;
    }
    if (percentage > 90) {
...
async function viewStudentDetails(username) {
    try {
        ...
        if (checkinRecords && checkinRecords.length > 0) {
            const dates = checkinRecords.map(record => record.clock_in_date);
            try {
                const { data: leaderboardData, error: leaderboardError } = await dbClient
                    .from('daily_leaderboard')
                    .select('leaderboard_date, base_score, quality_score, improvement_score')
                    .eq('student_id', username)
                    .in('leaderboard_date', dates);
                
                if (!leaderboardError && leaderboardData) {
                    leaderboardData.forEach(item => {
                        const totalScore = (item.base_score || 0) + (item.quality_score || 0) + (item.improvement_score || 0);
                        leaderboardScores[item.leaderboard_date] = totalScore;
                    });
                }
            } catch (error) {
                console.error('获取每日榜单分数错误:', error);
            }
        }
                            ...
                            <table style="width: 100%; border-collapse: collapse;">
                                ...
                                <tbody>
                                    ${checkinRecords.map(record => {
                                        const score = leaderboardScores[record.clock_in_date] || (record.score || 0);
                                        return `
                                            <tr style="border-bottom: 1px solid #f0f0f0;">
